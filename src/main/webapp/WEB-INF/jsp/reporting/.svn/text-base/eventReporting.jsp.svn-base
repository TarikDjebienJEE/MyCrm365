...
<div id="articleTab">
	<ul>
		<li><a href="#dayReport">Event evolution</a></li>
		<li><a href="#repartitionReport">Event repartition</a></li>
	</ul>
<section>
...
$('#loadReport').click(function() {
	...
	if (data[0].length > 0) {
		...
		$.jqplot(
			...
			{ 
				...
				title: 'Number of events integrated by day',
				seriesDefaults: { 
	...
	$('#hiddenRepartitionData input[name="key"]').each( function() {
  	...
  	if (repartitionData[0].length > 0) {
  
  		$.jqplot (
  			'repartitionReport', 
  			repartitionData, 
	    	{ 
	      	title: 'Event type repartition - all time',
	      	seriesDefaults: {
